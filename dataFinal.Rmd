...
```
```{r larval count across year and site, fig.cap= "D-hinge and veliger oyster larval counts by site. 2018 (Top), 2019 (Middle), 2020 (Bottom) are included.\\label{fig:site}"}
...
dSal <- dfD %>% 
  ...
  geom_point(alpha = 0.5, size = 0.5) +
  geom_smooth(method = "lm", fullrange = TRUE, color = "black") +
  labs(x = "Salinity", y = "D-hinge larval count") +
  stat_cor(size = 3) +
  theme_minimal() +
  theme(text=element_text(size=10))
